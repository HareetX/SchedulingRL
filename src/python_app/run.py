...
def train(num_episodes=4000, max_steps=10):
    ...
    # Plot rewards, episode_times, losses, actor_losses, critic_losses
    training_plot(rewards, episode_times, losses, actor_losses, critic_losses)
...
def training_plot(rewards, episode_times, losses, actor_losses, critic_losses):
    # Plot rewards
...
if __name__ == "__main__":
    if __name__ == "__main__":
        ...
        if len(sys.argv) == 5 and sys.argv[1] == "run":
            accelerator_path = sys.argv[2]
            network_path = sys.argv[3]
            layer_idx = int(sys.argv[4])
            if not os.path.exists(accelerator_path):
                print(f"Accelerator path {accelerator_path} does not exist.")
                sys.exit(1)
            if not os.path.exists(network_path):
                print(f"Network path {network_path} does not exist.")
                sys.exit(1)
            if layer_idx < 0 or layer_idx >= count_network_layers(network_path):
                print(f"Layer index {layer_idx} is out of range for network {network_path}.")
                sys.exit(1)
            run(
                accelerator_path=accelerator_path,
                network_path=network_path,
            ...
            print("Usage: python run.py [train|test|run <accelerator_path> <network_path> <layer_idx>]")
