...
std::vector<std::vector<unsigned>> mapping_space_t::get_mapping_set(unsigned index) {
    std::vector<std::vector<unsigned>> tmp_mapping_set;
    std::vector<std::vector<unsigned>> mapping_set;
    std::vector<unsigned> tmp_row_mapping;
    
    // Calculate parameter indices for the given index
    std::vector<unsigned> param_indices((unsigned)parameter_t::SIZE, 0);
    unsigned remaining_index = index;
    for (int i = (int)parameter_t::SIZE - 1; i >= 0; i--) {
        param_indices[i] = remaining_index % get_permutations(i).size();
        remaining_index /= get_permutations(i).size();
    }

    // Get the mapping set for the calculated indices
    for (unsigned i = 0; i < (unsigned)parameter_t::SIZE; i++) {
        tmp_mapping_set.push_back(get_permutations(i).at(param_indices[i]));
    }
